...
const UserProfileDash = () => {
  ...
  if (!user) return <div>No user is detected as logged in...</div>;
    ...
    <div className="container mx-auto">
      <div className="max-w-lg flex flex-col items-center relative mt-20 mx-auto border-red-400 py-6 bg-white shadow-md rounded-lg">
        ...
        <div className="flex items-center space-x-6 p-2 mb-6">
          <img
            className="w-24 h-24 rounded-full border-2 border-gray-300"
            src={user.profilePicture || DefultImg} // Use user's profile picture
            alt="Profile"
          />
          <div>
            <h2 className="text-2xl font-semibold text-gray-800">{user.name}</h2>
            <p className="text-gray-600 text-sm">{user.jobTitle || "User"}</p>
            <p className="text-gray-600 text-sm">Account Level: <span className='text-custom-orange'>{user.accountLevel || 'Premium'}</span></p>
          </div>
        </div>
            ...
            <div className="relative z-20">
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-gray-700">{user.location || 'New York, USA'}</p>
                </div>
                <div className="text-right">
                  <p className="text-gray-700">INVOICE NO #{selectedOrder.id}</p>
                  <p className="text-gray-700">DATE: {selectedOrder.date}</p>
